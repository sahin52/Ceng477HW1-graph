...
Vec3f Specular(PointLight light, RayIntersect ray, Scene scene, int cam_id, int mat_id){
    Vec3f recieved_irr = Irradiance(light, ray.intersectPoint);
    Vec3f w_i = normalize(Vec3fminus(light.position, ray.intersectPoint));
    Vec3f w_0 = Vec3fminus(scene.cameras[cam_id].position, ray.intersectPoint);

    float square_l = dotProduct(w_i, w_0);
    Vec3f half_v ;
    ...
    float cos = max(0.0f, dotProduct(ray.normal, half_v));
    float phong = scene.materials[mat_id].phong_exponent;

    return {
        .x = scene.materials[mat_id].specular.x * pow(cos, phong) * recieved_irr.x ,
        .y = scene.materials[mat_id].specular.y * pow(cos, phong) * recieved_irr.y ,
        .z = scene.materials[mat_id].specular.z * pow(cos, phong) * recieved_irr.z ,
    };
}
